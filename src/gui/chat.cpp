...
public:
    ChatWindow(boost::asio::io_context& aIoContext, const u_short &port ) : mConnection(aIoContext, port)
    {
    ...
    void Draw(const char* title, bool* p_open)
    {
        ...
        char addressBuff[256]{};
        ImGuiInputTextFlags address_text_flags = ImGuiInputTextFlags_EnterReturnsTrue;
        if(ImGui::InputText("Address", addressBuff, IM_ARRAYSIZE(addressBuff), address_text_flags))
        {
            mRemoteAddress = std::string(InputBuf);
        }
    ...
    void Send(const char* akMsg)
    {
        ...
        for (int i = History.Size - 1; i >= 0; i--)
            if (Stricmp(History[i], akMsg) == 0)
            {
                ImGui::MemFree(History[i]);
                History.erase(History.begin() + i);
                break;
            }
        History.push_back(Strdup(akMsg));

        // Process command
        if (Stricmp(akMsg, "CLEAR") == 0)
        {
            ClearLog();
        }
        else if (Stricmp(akMsg, "HELP") == 0)
        {
            AddLog("Commands:");
            for (int i = 0; i < Commands.Size; i++)
                AddLog("- %s", Commands[i]);
        }
        else if (Stricmp(akMsg, "HISTORY") == 0)
        {
